...
if (Test-Path -LiteralPath $PidFile) {
    ...
    if (-not $PidValue) {
        ...
        Write-Host "OpenClaw Gateway PID file was empty."
    }
    else {
        $Process = Get-Process -Id $PidValue -ErrorAction SilentlyContinue
        if (-not $Process) {
            Remove-Item -LiteralPath $PidFile -Force
            Write-Host "OpenClaw Gateway PID file was stale: process $PidValue was not found."
        }
        else {
    ...
    Write-Host "OpenClaw Gateway PID file not found."
}

$OrphanProcesses = @(Get-GatewayProcesses)
...
if ($StoppedPids.Count -eq 0) {
    Write-Host "OpenClaw Gateway is not running."
}
else {
    $StoppedPidList = @($StoppedPids) -join ', '
    Write-Host "OpenClaw Gateway stopped. PID(s): $StoppedPidList"
}
